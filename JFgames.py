...
def abrir_janela():
    ...
    botao_valor.grid(row=2, column=1, sticky="w", padx=10)
    botao_valor.configure(fg='white', bg='black')

    # Label para mensagens de cotação
    mensagem_cotacao = tk.Label(janela2, text='')
    mensagem_cotacao.grid(row=3, column=0, columnspan=2, sticky='w', padx=10)

    # Mensagem para reserva
    mensagem3 = tk.Label(janela2, text="Caso você queira reservar um produto que não tenha em nossa loja, insira aqui:")
    mensagem3.grid(row=4, column=0, sticky='w', padx=10, pady=(10, 0)) 
    mensagem3.configure(fg='white', bg='black')

    # Caixa de texto para reserva
    caixa_texto = tk.Text(janela2, width=30, height=5)
    caixa_texto.grid(row=5, column=0, columnspan=2, sticky="NSWE", padx=10)
    ...
    botao_fechar.grid(row=6, column=0, pady=10, sticky="w")
